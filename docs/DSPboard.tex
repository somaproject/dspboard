...
\subsubsubsection{0x0nnn: Status and  Data  Samples}
This DSP-read-only region contains new sample data and status
information.
\include{statusanddata.memmap}


\subsubsubsection{0x2nnn: Output data buffer} 
...
words).</para>
\include{outputdata.memmap}

\subsubsubsection{0x4nnn: Event Output} 

A write of an Event Bus event to this region queues it in the FPGA for
transmission on the event bus.
\include{eventoutput.memmap}

\subsubsubsection{0x6nnn : Event Input}
The DSP can read new events from the Event Bus here. 
\include{eventinput.memmap}

\subsubsubsection{0x8nnn: Acqboard interface}
This region allows us to send commands to the acquisition board. 
\include{acqboardio.memmap}
